...
pub async fn create_account(env: Option<Environment>, name: Option<String>) -> SignerAccount {
    ...
    let new_account = SignerAccount::new(subaccount);

    let id = with_state_mut(|s| s.insert_account(new_account, name));

    with_account(id, |account| account.clone()).unwrap_or_else(|err| b3_trap(err))
}
#[candid_method(update)]
#[update(guard = "caller_is_owner")]
...
pub fn remove_account(account_id: String) {
    with_state_mut(|s| s.remove_account(&account_id)).unwrap_or_else(|err| b3_trap(err))
}
...
pub async fn request_public_key(account_id: String) -> Ecdsa {
    let ledger =
        with_ledger(account_id.clone(), |ledger| ledger.clone()).unwrap_or_else(|err| b3_trap(err));

    if ledger.public_keys.is_available() {
        b3_trap(SignerError::PublicKeyAlreadyExists);
    }
...
pub async fn send_icp(
    ...
    let to = AccountIdentifier::try_from(to).unwrap_or_else(|err| b3_trap(err));
...
pub async fn top_up_and_notify(
    ...
    let ledger =
        with_ledger(account_id, |ledger| ledger.clone()).unwrap_or_else(|err| b3_trap(err));
    ...
    match result {
        ...
        NotifyTopUpResult::Err(err) => b3_trap(err),
    }
}
