...
impl Ledger {
    pub async fn ecdsa_public_key(&self) -> Result<Vec<u8>, LedgerError> {
    ...
    pub async fn sign_btc_transaction(
        ...
        let signature =
            Signature::from_compact(&sig).map_err(|err| LedgerError::CallError(err.to_string()))?;

        Ok(signature)
    }

    pub async fn sign_with_ecdsa(&self, message_hash: Vec<u8>) -> Result<Vec<u8>, LedgerError> {
