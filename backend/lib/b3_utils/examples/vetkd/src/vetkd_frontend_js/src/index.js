...
let app_backend_actor = vetkd_backend
let app_backend_principal = await Actor.agentOf(
  app_backend_actor
).getPrincipal()

let app_backend_subaccount = generateSubaccount(app_backend_principal)
...
async function ibe_encrypt(message) {
  ...
  const ibe_principal = Principal.fromText(
    document.getElementById("ibe_principal").value
  )

  let ibe_subaccount = generateSubaccount(ibe_principal)
...
document.getElementById("login").onclick = async e => {
  ...
  app_backend_subaccount = generateSubaccount(app_backend_principal)
...
function generateSubaccount(principal) {
