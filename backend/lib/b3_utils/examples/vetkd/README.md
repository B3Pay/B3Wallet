...
- An example app backend canister (`src/vetkd_backend`) implemented in **Rust** that makes use of this system API in order to provide caller-specific symmetric keys that can be used for AES encryption and decryption.

- An example frontend (`src/vetkd_frontend_js`) that uses the backend from Javascript in the browser.
...
1. Ensure the Canister SDK (dfx) uses the canister IDs that are hard-coded in the Rust source code:
   ...
   Without this, the Canister SDK (dfx) may use different canister IDs for the `system_api` and `vetkd_backend` canisters in your local environment.
...
1. Register, build and deploy the project:
   ...
   Frontend canister via browser
     vetkd_frontend_js: http://127.0.0.1:4943/?canisterId=by6od-j4aaa-aaaaa-qaadq-cai
   Backend canister via Candid interface:
     vetkd_backend: http://127.0.0.1:4943/?canisterId=avqkn-guaaa-aaaaa-qaaea-cai&id=tcvdh-niaaa-aaaaa-aaaoa-cai
...
1. Open the printed URL for the `vetkd_frontend_js` in your browser.
