...
impl Ledger {
    ...
    pub async fn bitcoin_balance(
        ...
        let address = self.keys.get_btc_address(network).unwrap();
    ...
    pub async fn bitcoin_get_utxos(
        ...
        let address = self.keys.get_btc_address(network)?;
    ...
    pub async fn bitcoin_transfer(
        ...
        let utxo_res = network.get_utxos(address.to_string(), None).await?;

        let utxo = BtcUtxo::try_from(utxo_res)?;

        let fee_rate = network.fee_rate(49).await?;

        let mut tx = utxo.build_transaction(&public_key, &dst_address, amount, fee_rate)?;
