...
pub struct BtcUtxo(Vec<Utxo>);

impl TryFrom<GetUtxosResponse> for BtcUtxo {
...
impl TryFrom<Vec<Utxo>> for BtcUtxo {
    type Error = WalletError;
...
impl BtcUtxo {
    pub fn build_transaction(
        ...
        let own_address = Address::p2wpkh(&public_key, bitcoin::Network::Regtest).unwrap();

        let mut total_fee = 0;
        loop {
            // 5. Create a new transaction with the calculated fee
            let mut transaction =
                self.build_transaction_with_fee(&own_address, recipient, amount, total_fee)?;

            // 6. Sign the new transaction
            for (_, input) in transaction.input.iter_mut().enumerate() {
                input.script_sig = mock_signer(&public_key);
                input.witness.clear();
            }

            // 7. Validate the new transaction
            let transaction_size = transaction.vsize() as u64;
            let extra_size = transaction.input.len() as u64; // Add 1 byte for each signature
...
mod test {
    ...
    use crate::ledger::{
        btc::{network::BtcNetwork, utxo::BtcUtxo},
        keys::Keys,
        types::AddressMap,
    };
    ...
    fn test_build_unsigned_transaction() {
        ...
        let utxos = BtcUtxo::try_from(vec![
            Utxo {
                outpoint: Outpoint {
                    txid: vec![
        ...
        let tx = utxos
            .build_transaction(&public_key, &recipient, 100_000_000, 2000)
            .unwrap();
