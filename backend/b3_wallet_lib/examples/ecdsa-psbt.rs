...
mod tests {
    ...
    fn test_psbt_workflow() {
        // Test data
        let ext_master_priv_key = "xprv9s21ZrQH143K3RLNfLaP2QrxPS6Ukf3MorGomhysTx8Twtdbn9dzfcMfhMbFeAzptdbewmFHtQXwdYK8GdxmFasDTn8Rq9su3sNQPxjzF3D";
        let ext_account_0_xpub = "xpub661MyMwAqRbcFuQqmN7PPYogwTvyA7mDB5CQa6PV2HfSpgxkKgxFDQg9Ybb9i14icN4XRoMqrDUfo1TrWsYbEcRd6eeQMG3hnNpyQBGjxLX";
        let ext_input_xpub = "xpub661MyMwAqRbcFuQqmN7PPYogwTvyA7mDB5CQa6PV2HfSpgxkKgxFDQg9Ybb9i14icN4XRoMqrDUfo1TrWsYbEcRd6eeQMG3hnNpyQBGjxLX";

        // Create the offline signer
        let secp = bitcoin::secp256k1::Secp256k1::new();
        let (offline, fingerprint, account_0_xpub, input_xpub) =
            ColdStorage::new(&secp, ext_master_priv_key).unwrap();
        let offline_wallet_data = (offline.clone(), fingerprint, account_0_xpub, input_xpub);

        // Create the watch-only online wallet
        let online = WatchOnly::new(
            ExtendedPubKey::from_str(ext_account_0_xpub).unwrap(),
            ExtendedPubKey::from_str(ext_input_xpub).unwrap(),
            offline_wallet_data.1,
        );

        // Create the PSBT
        let created_psbt = online.create_psbt(&secp).unwrap();
        assert!(!created_psbt.inputs.is_empty());

        // Update the PSBT
        let updated_psbt = online.update_psbt(created_psbt).unwrap();
        assert!(!updated_psbt.inputs.is_empty());

        // Sign the PSBT
        let signed_psbt = &offline.sign_psbt(&secp, updated_psbt).unwrap();
        assert!(!signed_psbt.inputs.is_empty());

        // Finalize the PSBT
        let finalized_psbt = online.finalize_psbt(signed_psbt.clone()).unwrap();
        assert!(!finalized_psbt.inputs.is_empty());

        // Extract the transaction from the finalized PSBT
        let transaction = finalized_psbt.clone().extract_tx();
        assert_eq!(transaction.input.len(), 1);
        assert_eq!(transaction.output.len(), 2);

        // Verify the transaction
        let verified = online.finalize_psbt(signed_psbt.clone()).unwrap();
        assert_eq!(verified, finalized_psbt);
    }

    #[test]
    fn test_generate_psbt() {
        let secp = Secp256k1::new();

        let (offline, fingerprint, account_0_xpub, input_xpub) =
            ColdStorage::new(&secp, EXTENDED_MASTER_PRIVATE_KEY).unwrap();

        let online = WatchOnly::new(account_0_xpub, input_xpub, fingerprint);

        let created = online.create_psbt(&secp).unwrap();
        let updated = online.update_psbt(created).unwrap();

        let signed = offline.sign_psbt(&secp, updated).unwrap();

        let finalized = online.finalize_psbt(signed).unwrap();

        let tx = finalized.extract_tx();
        let hex = encode::serialize_hex(&tx);

        // Verify the generated PSBT
        let expected_txid = "1fadb1dd2a564947facab1d2eb436b9da7408c99b7d0bd8ec1e83a32bbdb97e0";
        assert_eq!(tx.txid().to_string(), expected_txid);

        // Verify the change address
        let (_, change_address, _) = online.change_address(&secp).unwrap();

        let expected_change_address =
            Address::from_str("bcrt1qa8vnhzt4gjsjmln889urc0jnack5lean9kjnnn")
                .unwrap()
                .assume_checked();

        assert_eq!(change_address, expected_change_address);

        // Verify the serialized transaction hex
        let expected_hex = "";
